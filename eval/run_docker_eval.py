...
Usage (native / Colab):
  python run_docker_eval.py --project-root /path/to/StreamMind-LiveQA

Environment variables (alternative to CLI args):
  STREAMMIND_PROJECT_ROOT  - path to the project root
"""
...
def build_liveqa_bench(vlm: VLMEngine, force_rebuild: bool = False) -> list[QA]:
    ...
    to ensure deterministic results across runs.  Always call set_seed()
    before building so that Flan-T5 summarisation is reproducible.
    """
...
def main():
    ...
    import torch
    log.info("StreamMind Evaluation (v3 — deterministic, multi-metric)")
    device = "cuda" if torch.cuda.is_available() else "cpu"
