...
It contains 50 simulated live streams (5 to 30 minutes each), constructed by joining clips from Ego4D~\cite{grauman2022ego4d}---a large-scale egocentric video dataset spanning thousands of hours of daily activities---and Kinetics action recognition clips with natural transitions.
The use of Ego4D clips ensures that LiveQA-Bench inherits realistic egocentric scenarios (cooking, object manipulation, navigation) that demand the kind of temporal reasoning absent from static VideoQA benchmarks.
...
We evaluate on a 63-question subset of LiveQA-Bench (2 streams covering cooking and surveillance domains, with all three scope types well represented). All \ours results are reported on an NVIDIA T4 GPU (Google Colab) with fixed random seeds for reproducibility; the full 500-question evaluation will be included in the public release.
