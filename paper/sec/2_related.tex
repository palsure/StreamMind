...
The egocentric regime is particularly relevant to streaming QA: body-mounted cameras produce a continuous unscripted stream where event density varies dramatically---from idle walking to rapid object manipulations.
Ego4D's Natural Language Queries (NLQ) task, which grounds temporal queries to specific video moments, shares our goal of temporal localization but assumes offline access to the full recording.
\ours builds on Ego4D clips as source material for LiveQA-Bench and inherits the diversity of egocentric scenarios, but operates under a strict causal constraint where future frames are never available.
