...
This dynamic re-scoring is the key difference from one-pass selection methods such as SeViLA's keyframe localization~\cite{yu2023sevila}, which commit to a fixed set once.

\paragraph{Temporal bookkeeping.}
Each memory entry stores its original timestamp along with a compact JPEG thumbnail (used for the demo filmstrip) and its CLIP embedding (used for novelty computation). The TQR uses these timestamps to filter entries by time window. We sort memory by timestamp after each insertion so that scope-filtered retrieval (\cref{sec:tqr}) requires a single binary search.
...
A question like \emph{``What just happened?''} is \textsc{recent} regardless of whether the stream shows one event per minute (surveillance) or ten events per second (sports replay).
...
When $W = 30$\,s, the \textsc{recent} scope captures roughly the last 3--5 events in a high-density cooking stream (Ego4D-style egocentric video~\cite{grauman2022ego4d}) or the last 0--1 events in a slow surveillance feed.
This asymmetry has two consequences.
First, for dense streams the \textsc{recent} window may be saturated with keyframes, and the SFG's uniform sampling (\cref{sec:sfg}) selects a representative subset.
Second, for sparse streams the window may be empty; the conservative default to \textsc{historical} then ensures the system searches the full memory rather than returning an empty context.

We chose a single $W$ for simplicity.
An adaptive scheme that estimates event density from the SKM insertion rate---for example, $W_{\text{adapt}} = k / \lambda$ where $\lambda$ is the average insertions per second---could refine this boundary automatically, but in our experiments the fixed value works well across all evaluated domains (\cref{sec:experiments}).
We leave adaptive window estimation as future work.
...
Flan-T5-base~\cite{chung2022flanT5} generates up to 80 tokens with beam search (4 beams, length penalty 1.0, no-repeat 3-gram constraint).
...
End-to-end latency stays under 200\,ms on a single NVIDIA A100 (estimated from component timings; see \cref{sec:latency} for measured T4 numbers). The two BLIP models (captioning and VQA) share the same vision backbone, so the feature extraction cost is amortized across both tasks.
